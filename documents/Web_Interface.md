...
│   └── sqlite.py       # /api/sqlite/fetch-missing
...
```json
{
    ...
    "connection": {
        ...
        "reconnect_count": 0,          // 重連次數
        "last_check": "2026-03-09T19:30:28.071157"
    },
    "sqlite": {...},
    "sqlite_total": 483807,
    "message": "..."
...
**說明：**
- `connection.status`：連線狀態（正常/中斷/重連中）
- `connection.reconnect_count`：重連次數（重連成功後歸零）
- Web 首頁動態顯示連線狀態圖示（✅/❌）
...
系統總覽頁面顯示 SQLite 數據狀態，包含：

| 欄位 | 說明 |
|------|------|
| 資料筆數 | 該 symbol 的總 K 棒數量 |
| 總天數 | 資料筆數 / 平均每日筆數（約 830 筆/天）|
| 最新日期 | 資料中最後一筆的日期 |
| 工作日缺口 | 整日資料缺失的天數（需補抓）|
| 交易時段異常 | 當日筆數低於平均 95% 的天數 |

### 9.2 手動補抓缺失數據

點擊「🔄 獲取缺失數據」按鈕可手動補抓缺失的數據。

**功能**：
- 檢查工作日缺口日期 → 調用 API 補抓
- 檢查交易時段異常日期 → 調用 API 補抓
- 自動跳過已確認無資料的日期（假日）
- 自動跳過已有資料的日期
- 將補抓結果記錄到 fetch_log 表

### 9.3 API 端點

| 方法 | 路徑 | 說明 |
|------|------|------|
| POST | `/api/sqlite/fetch-missing` | 手動補抓缺失數據 |

**請求**：
```bash
curl -X POST http://127.0.0.1:5001/api/sqlite/fetch-missing
```
...
```json
{
  "success": true,
  "result": {
    "symbols_updated": ["MXFR1", "TMFR1", "TXFR1"],
    "total_fetched": 1234,
    "dates_checked": 5,
    "no_data_dates": ["2026-02-13", "2026-02-14"],
    "errors": []
  }
}
```
...
| 類型 | 意義 | 處理方式 |
|------|------|----------|
| 工作日缺口 | 整日資料缺失 | 嘗試從 API 補抓 |
| 交易時段異常 | 當日筆數低於平均 95% | 嘗試從 API 補抓 |
| fetch_log success | 已有資料（無論數量）| 排除，不顯示異常 |
| fetch_log no_data | 已確認無資料（假日）| 排除，不顯示異常 |
