...
| **4.1.0** | **2026-03** | **新增交易日誌系統（TradeLogStore），系統總覽顯示重要交易訊息** |
| **4.2.0** | **2026-03** | **統一平倉流程，確保所有平倉操作都記錄訂單與日誌** |
| **4.3.0** | **2026-03** | **系統日誌重命名（trading.log → system.log），訂單查詢新增原因欄位** |
| **4.4.0** | **2026-03** | **新增啟用策略確認機制（Scheme C），舊策略有部位時需確認強制平倉** |
| **4.5.0** | **2026-03** | **模擬模式價格生成改為趨勢模擬演算法，支援 RSI/MACD 指標正常運作** |
| **4.6.0** | **2026-03** | **MockContract 使用部位進場價作為 last_price，避免系統重啟時誤觸停損** |
| **4.7.0** | **2026-03** | **統一停損檢查價格來源：從 strategy_runner 獲取，確保與策略決策使用相同價格** |
| **4.8.0** | **2026-03** | **訂單價格類型預設改為 MKT（市價單），與實盤行為及文檔一致** |
| **4.9.0** | **2026-03** | **刪除策略時連帶刪除所有 backtest 檔案（HTML、TXT、PNG）** |
| **4.10.0** | **2026-03** | **模擬價格統一化：回測K線(_generate_mock_kbars)與實時交易(_simulate_price_updates)統一使用 ShioajiClient.TIMEFRAME_VOLATILITY，根據策略 timeframe 自動調整波動率(1m:0.03%~1d:1.2%)** |
| **4.6.1** | **2026-03-01** | **Web Interface 改進：當日損益顯示已實現+未實現損益、策略創建時可自訂名稱、簡化命名邏輯、口數驗證(>=1)、停損止盈允許為0(不啟用)、交易方向修復、Stage 1 Review 日誌鏈接、按鈕文字改為「讓 LLM 設計策略」** |
