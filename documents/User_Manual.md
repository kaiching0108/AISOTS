...
4. 確認參數後點擊「確認建立」
5. 系統會先檢查策略程式碼編譯是否成功，若失敗會自動嘗試修復一次
6. 編譯成功後，系統會執行兩階段驗證：
   - Stage 1：LLM 自我審查
   - Stage 2：歷史回測
7. 驗證通過後，策略建立成功
...
3. 點擊「回測」按鈕
4. 系統檢查是否有現有回測報告：
   - **有現有報告**：顯示對話框，選擇「查看報告」或「執行新回測」
   - **無現有報告**：直接執行回測
5. 執行回測前，檢查 SQLite 數據是否足夠：
   - **數據充足**：直接使用 SQLite 數據執行回測
   - **數據不足**：顯示對話框，選擇：
     - 「使用模擬數據」：使用模擬 K-bar（不消耗 API 流量）
     - 「使用現有數據」：使用 SQLite 現有有限範圍數據
6. 顯示回測結果：
...
當發現有數據缺失時，可以點擊「🔄 獲取缺失數據」按鈕手動補抓。

**系統會**：
- 檢測工作日缺口日期 → 調用 API 補抓
- 檢測交易時段異常日期 → 調用 API 補抓
- 自動跳過已確認無資料的日期（假日）
- 自動跳過已有資料的日期
- 將補抓結果記錄到資料庫

**補抓結果**：
- **success**：成功獲取資料（無論數量多少）
- **no_data**：已確認該日無資料（通常是假日）
- **error**：API 調用失敗
...
|------|------|-------------|
| 工作日缺口 | 整日資料缺失 | 是 |
| 交易時段異常 | 當日筆數低於平均 95% | 是 |
| fetch_log success | 已有資料 | 否（已確認）|
| fetch_log no_data | 已確認無資料（假日）| 否（已確認）|
...
A：可能是因為：
1. 該日是假日（無交易）- 補抓後會標記為 no_data
2. 該日提前收盤 - 補抓後會標記為 success，但筆數仍然較少

**Q：如何知道哪些日期已確認無資料？**

A：補抓後會自動記錄到 fetch_log，下次檢查時會自動排除。
...
系統每次價格更新時檢查部位：
- **價格來源**：從報價快取 `_latest_prices` 獲取最新市場價格，透過 `get_latest_price()` 統一介面取得
- 虧損 >= 停損點數 → 自動平倉
- 獲利 >= 止盈點數 → 自動平倉
- 檢查頻率：依市場報價頻率
...
**模擬模式說明**：
- 使用趨勢模擬演算法生成連續價格
- `simulation: true` 時仍會訂閱報價（只是下單為虛擬單）
