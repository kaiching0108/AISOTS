...
所有明確英文指令透過 **Fallback** 直接調用函數，不依賴 LLM 工具調用，確保執行成功：

| 命令 | 說明 | 範例 |
|------|------|------|
| `create` | 問答式建立策略 | create |
| `status` | 系統狀態 | status |
...
| `help` / `幫助` | 顯示命令列表 | help |
| `enable <id>` | 啟用策略 | enable TMF260001 |
| `disable <id>` | 停用策略 | disable TMF260001 |
| `回測 <id>` / `backtest <id>` | 執行歷史回測 | 回測 TMF260001 |
...
| 範圍 | 說明 | 範例 |
|------|------|------|
| 目標驅動建立策略 | 給出目標，LLM 自動推斷參數 | 幫我設計一個每日賺500元的策略 |
| 策略討論 | 與 LLM 討論策略邏輯 | 這個策略的 RSI 參數怎麼設定比較好？ |

### 6.3 績效分析命令

| 命令 | 說明 | 範例 |
|------|------|------|
| `performance <ID>` | 查詢策略全部歷史績效 | performance TMF260001 |
| `performance <ID> today` | 查詢策略今日績效 | performance TMF260001 today |
| `performance <ID> week` | 查詢策略本週績效 | performance TMF260001 week |
| `performance <ID> month` | 查詢策略本月績效 | performance TMF260001 month |
| `performance <ID> quarter` | 查詢策略本季績效 | performance TMF260001 quarter |
| `performance <ID> year` | 查詢策略本年績效 | performance TMF260001 year |
...
| 命令 | 說明 | 範例 |
|------|------|------|
| `回測 <ID>` | 執行歷史回測（中文）| 回測 TMF260001 |
| `backtest <ID>` | 執行歷史回測（英文）| backtest TMF260001 |
...
執行回測後，系統會先發送文字報告，隨後發送一張回測圖表。

```
用戶: 回測 TMF260001

Bot: 📊 歷史回測報告 (RSI策略_TXF)
...（文字報告）
...
Bot: 📊 歷史回測報告 (RSI策略_TXF)
══════════════════════════════════════════
📅 回測期間: 2025-01-15 ~ 2025-02-15 (1個月)
📈 初始資金: 1,000,000 NTD
─────────────────────────────────────────
💰 總損益: +85,000 (+8.5%)
💵 最大資金回撤: -32,000 (-3.2%)
...
系統使用 **pandas_ta** 預先計算指標，再傳給 backtrader 執行回測。支援的指標包括：
- RSI
- MACD
- SMA / EMA
- 布林帶 (BB)
...
2. **未通過驗證的策略無法回測**：需先通過策略驗證才能執行回測
3. **回測期間依 timeframe 而定**：分鐘線回測期間較短，日線回測期間較長

### 6.5 自我優化命令

| 命令 | 說明 | 範例 |
|------|------|------|
| `goal <ID> <金額> <單位>` | 設定策略目標 | goal TMF260001 500 daily |
| `review <ID>` | LLM 審查策略並給出修改建議 | review TMF260001 |
| `optimize <ID>` | 執行完整優化流程 | optimize TMF260001 |
...
範例:
goal TMF260001 500 daily      # 每日賺500元
goal TMF260001 3000 weekly    # 每週賺3000元
goal TMF260001 10000 monthly  # 每月賺10000元
```
...
```
用戶: performance TMF260001 month
Bot: 📈 策略績效: TMF260001 (2025-02-01 ~ 2025-02-18)
     ─────────────────────────────
     合約: TXF
