...
class TestFallbackCommands:
    ...
    def test_delete_nonexistent_strategy(self, trading_tools_with_mocks):
        """測試刪除不存在的策略"""
        # Mock 策略不存在
        trading_tools_with_mocks.position_mgr.get_position.return_value = None
        result = trading_tools_with_mocks.delete_strategy_tool("TMF260001")
        assert "找不到" in result or "❌" in result
    
    def test_delete_strategy_with_position(self, trading_tools_with_mocks):
        """測試刪除有部位的策略"""
        strategy = Mock()
        strategy.name = "TestStrategy"
        trading_tools_with_mocks.strategy_mgr.get_strategy.return_value = strategy
        
        position = Mock()
        position.quantity = 1
        position.symbol = "TXF"
        position.direction = "Buy"
        position.entry_price = 18000
        trading_tools_with_mocks.position_mgr.get_position.return_value = position
        
        result = trading_tools_with_mocks.delete_strategy_tool("TMF260001")
        assert "確認" in result or "confirm delete" in result.lower()
    
    def test_delete_strategy_success(self, trading_tools_with_mocks):
        """測試成功刪除策略"""
        strategy = Mock()
        strategy.name = "TestStrategy"
        trading_tools_with_mocks.strategy_mgr.get_strategy.return_value = strategy
        trading_tools_with_mocks.position_mgr.get_position.return_value = None
        trading_tools_with_mocks.strategy_mgr.delete_strategy.return_value = True
        
        result = trading_tools_with_mocks.delete_strategy_tool("TMF260001")
        assert "已刪除" in result
    
    def test_delete_strategy_with_position_prompt(self, trading_tools_with_mocks):
        """測試刪除有部位的策略時會詢問確認"""
        strategy = Mock()
        strategy.name = "TestStrategy"
        trading_tools_with_mocks.strategy_mgr.get_strategy.return_value = strategy
        ...
        result = trading_tools_with_mocks.delete_strategy_tool("TMF260001")
        assert "confirm delete" in result.lower() or "確認" in result
    
    def test_confirm_delete_without_pending(self, trading_tools_with_mocks):
        """測試直接輸入 confirm delete（無 pending）"""
        strategy = Mock()
        strategy.name = "TestStrategy"
        trading_tools_with_mocks.strategy_mgr.get_strategy.return_value = strategy
        trading_tools_with_mocks.position_mgr.get_position.return_value = None
        
        result = trading_tools_with_mocks.confirm_delete_strategy("TMF260001")
        assert "已刪除" in result
    
    def test_confirm_delete_with_position(self, trading_tools_with_mocks):
        """測試 confirm delete 有部位的策略"""
        strategy = Mock()
        strategy.name = "TestStrategy"
        trading_tools_with_mocks.strategy_mgr.get_strategy.return_value = strategy
        ...
        trading_tools_with_mocks.api.get_contract.return_value = Mock(last_price=18500)
        trading_tools_with_mocks.api.place_order = Mock()
        trading_tools_with_mocks.position_mgr.close_position = Mock()
        
        result = trading_tools_with_mocks.confirm_delete_strategy("TMF260001")
        assert "已刪除" in result or "強制平倉" in result
